...
final class ReportTests: XCTestCase {
    ...
    func testAutomaticDiagnosticsExportIsAvailableWithoutPcmOrArtwork() throws {
        let presentation = LifecyclePresentation(
            state: .visualizing,
            message: "Live signal available.",
            consentIntent: true,
            isWindowVisible: true,
            playback: PlaybackObservation(state: .playing)
        )
        let export = SpaceVisualizerDiagnosticsExport(
            appBuild: "test",
            presentation: presentation,
            display: DisplayTelemetrySnapshot(
                frameWork: LatencySummary(samples: 2, p50Nanoseconds: 1_000, p95Nanoseconds: 2_000)
            )
        )
        let json = String(decoding: try export.encodedJSON(), as: UTF8.self)

        XCTAssertTrue(json.contains("visualizing"))
        XCTAssertTrue(json.contains("frameWork"))
