...
final class ConfigurationContractTests: XCTestCase {
    ...
    func testDefaultShellPrioritizesCanvasAndKeepsDiagnosticsOptional() throws {
        ...
        let commands = try String(contentsOf: root.appendingPathComponent("Sources/SpaceVisualizerApp/VisualizerCommands.swift"), encoding: .utf8)
        XCTAssertTrue(commands.contains("CommandMenu(\"Visualizer\")"))
        XCTAssertTrue(commands.contains("Button(\"Export Image…\") { menuActions?.exportImage() }"))
        XCTAssertTrue(commands.contains(".disabled(menuActions == nil)"))
        XCTAssertTrue(commands.contains("accessibilityHint"))
        XCTAssertTrue(contentView.contains(".focusedSceneValue(\\.visualizerMenuActions, menuActions)"))
        XCTAssertTrue(contentView.contains(".onChange(of: menuActions.imageExportRequest)"))
        XCTAssertFalse(contentView.contains("Label(\"Export image\""))
