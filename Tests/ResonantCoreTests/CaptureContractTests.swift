...
final class CaptureContractTests: XCTestCase {
    ...
    func testRouteResolverRequiresExplicitSelectionForAmbiguousSources() throws {
        let provider = FakeAudioRouteProvider(routes: [.fixtureSpeaker, .fixtureAirPlay])
        let resolver = AudioRouteResolver(provider: provider)

        XCTAssertEqual(resolver.resolve(preferredID: nil), .ambiguous(["Built-in Speakers", "Living Room AirPlay"]))
        XCTAssertEqual(resolver.resolve(preferredID: "fixture-speaker"), .selected(.fixtureSpeaker))
    }
