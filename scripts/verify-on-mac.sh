...
LOG="$ROOT/.build/resonant-verification.log"
mkdir -p "$ROOT/.build"
cd "$ROOT"
...
{
    echo "Resonant macOS verification"
    ...
    "$ROOT/scripts/generate-control-fixture.sh"
    ls -lh "$ROOT/.build/ResonantControl.wav"
    echo
    echo "=== App bundle build ==="
    "$ROOT/scripts/build-resonant-app.sh"
    echo
    echo "Built app bundle: $ROOT/.build/Resonant.app"
    echo "Launch manually with: open \"$ROOT/.build/Resonant.app\""
    echo
