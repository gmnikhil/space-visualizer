...
4. The script reads `player state` first. Each current-track field is read in an independent `try` block. If the combined response is malformed, contains an unrecognized playback state, exceeds the output limit, or the helper reports a generic error (including descriptor errors), the adapter makes one independent state-only query without accessing current-track metadata. It rechecks Music's presence first and gives this fallback a half-second execution timeout. A successful fallback returns playback state without track details; an invalid fallback remains a failure. Denial, timeout, and cancellation do not trigger the fallback.
5. The coordinator enforces a two-second end-to-end deadline spanning the original request and any fallback, including queueing and helper launch. It retires expired requests before cancellation, ignores late callbacks, and allows subsequent polling to proceed.
6. Cancellation tokens reference helper executions weakly. Launch/exit work keeps each execution alive until its process exits and output drains; completion releases its callback and process reference. This prevents completed polling requests from accumulating through retain cycles. Cancellation and launch are serialized.
7. An unreadable state-only response is included in the failure diagnostic as an escaped preview of at most 160 characters. Combined track-metadata responses are not included.

During active visualization, the first failed playback check freezes the displayed position at its extrapolated value. Transient check failures appear only in playback-check diagnostics, without replacing the LIVE status or adding an uncertainty message. Capture and the active watchdog survive the first two consecutive failures. The third failure releases capture and enters five-second recovery polling with “Lost connection to Music” status. Further failures do not disable polling. Any successful observation resets the failure count and replaces the frozen position with Music's reported position. Confirmed pause/stop and permission denial still take effect immediately; denial never enters automatic recovery polling. Independent audio-input failure deadlines remain in effect.
