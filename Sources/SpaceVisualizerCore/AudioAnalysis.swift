...
public struct AudioRingWriteResult: Equatable, Sendable {
    public let writtenSamples: Int
    public let droppedOldestSamples: Int
    public let rejectedSamples: Int

    public init(writtenSamples: Int, droppedOldestSamples: Int, rejectedSamples: Int = 0) {
        self.writtenSamples = writtenSamples
        self.droppedOldestSamples = droppedOldestSamples
        self.rejectedSamples = rejectedSamples
    }

    public var didDropSamples: Bool {
        droppedOldestSamples > 0 || rejectedSamples > 0
    }
}
...
public final class AudioRingBuffer: @unchecked Sendable {
    ...
    public func write(_ samples: UnsafeBufferPointer<Float>) -> Int {
        ...
        lock.lock()
        defer { lock.unlock() }
        return writeLocked(samples).droppedOldestSamples
    }

    /// The capture callback uses this nonblocking form. A contended consumer
    /// lock rejects the complete bounded input block instead of waiting.
    public func tryWrite(_ samples: UnsafeBufferPointer<Float>) -> AudioRingWriteResult {
        guard !samples.isEmpty else {
            return AudioRingWriteResult(writtenSamples: 0, droppedOldestSamples: 0)
        }
        guard lock.try() else {
            return AudioRingWriteResult(writtenSamples: 0, droppedOldestSamples: 0,
                                        rejectedSamples: samples.count)
        }
        defer { lock.unlock() }
        return writeLocked(samples)
    }

    private func writeLocked(_ samples: UnsafeBufferPointer<Float>) -> AudioRingWriteResult {
        var dropped = 0
        ...
        return AudioRingWriteResult(writtenSamples: samples.count, droppedOldestSamples: dropped)
    }
    ...
    public func readLatestWindow(sampleCount: Int, retainingSamples: Int = 0) -> [Float] {
        guard sampleCount > 0, sampleCount <= capacity,
              retainingSamples >= 0, retainingSamples < sampleCount,
              retainingSamples <= capacity else { return [] }
        lock.lock()
        defer { lock.unlock() }
    ...
    /// Nonblocking reset for a real-time producer. The logical reset is
    /// constant-time and never touches the backing storage.
    public func tryReset() -> Bool {
    ...
    private func resetLocked() {
        ...
        writeIndex = 0
        storedCount = 0
        // Old slots are overwritten before they can be read.
    }
}

public struct PCMBufferCollectorSnapshot: Equatable, Sendable {
    ...
    public init(acceptedSamples: UInt64, droppedSamples: UInt64,
                discontinuities: UInt64, pendingSamples: Int) {
...
public final class PCMBufferCollector: @unchecked Sendable {
    ...
    private var latestTimestampValue: UInt64 = 0
    ...
    private var acceptedSamplesValue: UInt64 = 0
    private var droppedSamplesValue: UInt64 = 0
    ...
    public var latestTimestamp: UInt64 {
        metadataLock.lock()
    ...
    /// PCM and its timestamp/format metadata commit while one lock is held.
    @discardableResult
    public func accept(
        ...
        guard !samples.isEmpty, sampleRate.isFinite, sampleRate > 0,
              channelCount > 0, samples.count % channelCount == 0 else {
            return samples.count
        }
        // The producer never waits for the worker or for a competing callback.
        guard metadataLock.try() else { return samples.count }
        defer { metadataLock.unlock() }

        let changedFormat = channelCountValue > 0 &&
            (sampleRateValue != sampleRate || channelCountValue != channelCount)
        let changedGeneration = channelCountValue > 0 && generationValue != generation
        let timestampRegressed = latestTimestampValue > 0 && timestamp <= latestTimestampValue
        if changedFormat || changedGeneration || timestampRegressed || resetPending {
            guard buffer.tryReset() else {
                resetPending = true
                discontinuityPending = true
                droppedSamplesValue &+= UInt64(samples.count)
        ...
        let result = buffer.tryWrite(samples)
        droppedSamplesValue &+= UInt64(result.droppedOldestSamples + result.rejectedSamples)
        guard result.writtenSamples > 0 else {
            discontinuityPending = true
            discontinuityCount &+= 1
            return samples.count
        }
        if result.didDropSamples {
            discontinuityPending = true
            discontinuityCount &+= 1
        }
        ...
        acceptedSamplesValue &+= UInt64(result.writtenSamples)
        return result.droppedOldestSamples + result.rejectedSamples
    }

    /// Marks a producer discontinuity without blocking the real-time callback.
    public func markDiscontinuity() {
        guard metadataLock.try() else { return }
        defer { metadataLock.unlock() }
        resetPending = !buffer.tryReset()
        discontinuityPending = true
        discontinuityCount &+= 1
    }
    ...
    public func latestWindow(fftSize: Int, hopSize: Int) -> AudioFrame? {
        metadataLock.lock()
        defer { metadataLock.unlock() }
        guard channelCountValue > 0, fftSize > 0, !resetPending,
              fftSize * channelCountValue <= buffer.capacity else { return nil }
        let count = fftSize * channelCountValue
        let hop = min(fftSize, hopSize) * channelCountValue
        ...
        let frame = AudioFrame(
            timestamp: latestTimestampValue,
            sampleRate: sampleRateValue,
            channelCount: channelCountValue,
            generation: generationValue,
            samples: samples,
            isFresh: latestTimestampValue > 0,
            isDiscontinuous: discontinuityPending
        )
        discontinuityPending = false
        return frame
    }

    public func snapshot() -> PCMBufferCollectorSnapshot {
        metadataLock.lock()
        defer { metadataLock.unlock() }
        return PCMBufferCollectorSnapshot(
            acceptedSamples: acceptedSamplesValue,
            droppedSamples: droppedSamplesValue,
            discontinuities: discontinuityCount,
            pendingSamples: buffer.count
        )
    }
...
public final class AudioFrameMailbox: @unchecked Sendable {
    ...
    public init(generation: UInt64? = nil) {
        ...
        if let generation { self.generationValue = generation }
    }
    ...
    @discardableResult
    public func publish(_ frame: AudioFrame) -> Bool {
        lock.lock()
        defer { lock.unlock() }
        if let expectedGeneration {
            guard frame.generation == expectedGeneration else { return false }
        } else {
            expectedGeneration = frame.generation
            generationValue = frame.generation
        }
        self.frame = frame
        return true
    }
...
public struct AudioAnalyzerConfiguration: Equatable, Sendable {
    ...
    public var isSupported: Bool {
        sampleRate.isFinite && sampleRate > 0 && channelCount > 0 &&
            isInterleaved && sampleFormat == .float32
    }

    public var unsupportedReason: String? {
        if !sampleRate.isFinite || sampleRate <= 0 { return "Capture sample rate is invalid." }
        if channelCount <= 0 { return "Capture channel layout is invalid." }
        if !isInterleaved { return "Planar PCM is not supported by the direct tap." }
        if sampleFormat != .float32 { return "Only packed interleaved Float32 PCM is supported." }
        return nil
    }
...
public final class AudioAnalyzer {
    ...
    private func makeMonoFrame(from samples: [Float]) -> [Float] {
        ...
        if configuration.isInterleaved {
            ...
            for index in 0..<configuration.fftSize where frameStart + index < samples.count {
                let sample = samples[frameStart + index]
                mono[index] = sample.isFinite ? sample : 0
            }
        }
        return mono
    }
    ...
    private func rootMeanSquare(_ values: [Float]) -> Float {
        ...
        return sqrt(max(0, sum / Float(values.count)))
    }
...
public struct FeatureSmoother: Sendable {
    ...
    public let referenceIntervalNanoseconds: UInt64
    ...
    public init(
        attack: Float = 0.35,
        release: Float = 0.12,
        referenceIntervalNanoseconds: UInt64 = AudioTiming.smoothingReferenceIntervalNanoseconds
    ) {
        self.attack = min(1, max(0, attack))
        self.release = min(1, max(0, release))
        self.referenceIntervalNanoseconds = max(1, referenceIntervalNanoseconds)
    }

    /// Applies an exponential time constant. Passing a real elapsed interval
    /// keeps attack/release stable when worker cadence or display cadence
    /// changes; the default preserves the historical one-step API for callers
    /// that do not have an input timestamp.
    public mutating func ingest(
        _ next: AudioFeatures,
        elapsedNanoseconds: UInt64? = nil
    ) -> AudioFeatures {
        if let lastGeneration, lastGeneration != next.generation {
            reset()
        }
        lastGeneration = next.generation

        guard next.isFresh else {
            // A stale frame cannot keep the previous visual signal alive. Reset
            // both levels and smoothing memory before exposing the status.
        ...
        let elapsed = max(1, elapsedNanoseconds ?? referenceIntervalNanoseconds)
        func coefficient(_ base: Float) -> Float {
            guard base > 0 else { return 0 }
            if base >= 1 { return 1 }
            let exponent = Double(elapsed) / Double(referenceIntervalNanoseconds)
            return Float(1 - pow(1 - Double(base), exponent))
        }
        func approach(_ old: Float, _ new: Float) -> Float {
            let oldValue = old.isFinite ? old : 0
            let newValue = new.isFinite ? new : 0
            let amount = coefficient(newValue > oldValue ? attack : release)
