...
public final class DiagnosticEngine: ObservableObject {
    ...
    private var lastDiagnosticTime: UInt64 = 0
    ...
    public static func live() -> DiagnosticEngine {
        DiagnosticEngine(
            metadataQuery: AppleScriptMusicQuery(),
    ...
    public func start(routeID: String?) {
        ...
        do {
            ...
            self.pipeline = AudioFeaturePipeline(collector: collector, analyzer: AudioAnalyzer(
                configuration: AudioAnalyzerConfiguration(sampleRate: format.sampleRate,
                    channelCount: format.channelCount, isInterleaved: format.isInterleaved,
    ...
    public func pollFeatures() {
        ...
        if now - lastTelemetryTime >= 250_000_000, let capture {
            captureDiagnostics = capture.telemetry.snapshot().summary
        ...
        if now - lastDiagnosticTime >= 250_000_000 {
    ...
    public var buildContext: String {
        ...
        return "Resonant feasibility spike \(version)"
    }
